...
x = x[2:]
#x = [i[1].lower().replace(' ', '_')\
#     +' = '+i[1].replace(' ', '')\
#     +' = 0x'+i[0][1:].lower() for i in x]

longest_color = 0
for i in x:
    longest_color = max(longest_color, len(i[1]))
print(longest_color)
for i in x:
    under = i[1].lower().replace(' ', '_')
    while len(under) < longest_color:
        under += ' ' 
    camel = i[1].replace(' ', '')
    while len(camel) < longest_color:
        camel += ' ' 
    print(' = '.join([under,camel,'0x'+i[0][1:].lower()]))
